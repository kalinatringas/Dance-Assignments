...
class DanceScheduler:
    ...
    @classmethod
    def from_dataframes(cls, dancers_df: pd.DataFrame, capacities_df: pd.DataFrame = None):
        """Create a DanceScheduler from pandas DataFrames (used by the API)."""
        data = dancers_df.copy()
        data.columns = data.columns.str.strip()
        ...
        if capacities_df is not None:
            cap = capacities_df.copy()
            cap.columns = cap.columns.str.strip()
            for __, row in cap.iterrows():
                dance_name = row.get("Dance Name")
                if pd.isna(dance_name):
                    continue
                dance_name = str(dance_name).strip()
                try:
                    capacity = int(row.get("Dancer Count"))
                except Exception:
                    # if capacity is missing or malformed, skip
                    continue
                capacities[dance_name] = capacity
    ...
    def generate_configurations(self, n:int=5, num_trials: int = None ) -> List[Dict[str, List[str]]]:
        configs = []
        # support callers that pass num_trials instead of n
